...
void MainWindow::createActions()
{
	...
	actInsertChars = new QAction(tr("Insert chars"));
	actInsertChars->setObjectName("actInsertChars");
	connect(actInsertChars, &QAction::triggered, this, &MainWindow::onInsertChar);
	actInsertDate = new QAction(tr("Insert date"));
	actInsertDate->setObjectName("actInsertDate");
	connect(actInsertDate, &QAction::triggered, this, [this](){this->onInsertDate(true);});
	actInsertTime = new QAction(tr("Insert time"));
	actInsertTime->setObjectName("actInsertTime");
	connect(actInsertTime, &QAction::triggered, this, [this](){this->onInsertDate(false);});
...
void MainWindow::createMenu()
{
	...
	menuInsert = menuBar()->addMenu(tr("Insert"));
	menuInsert->addAction(actInsertChars);
	menuInsert->addSeparator();
	menuInsert->addAction(actInsertDate);
	menuInsert->addAction(actInsertTime);
...
void MainWindow::onInsertChar()
{
	DialogInsertChar dialog;
	if (dialog.exec() == QDialog::Accepted) {
	}
}

void MainWindow::onInsertDate(bool isDate)
{
	QDateTime datetime = QDateTime::currentDateTime();;
	//std::cout << datetime.toString().toStdString() << std::endl;
	if (isDate)
		textEditMain->textCursor().insertText(datetime.date().toString());
	else
		textEditMain->textCursor().insertText(datetime.time().toString());
}
